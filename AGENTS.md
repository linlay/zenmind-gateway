...
- `upstream`: 上游服务地址（如 `http://agent-platform:8080/api/`）
...
- `GET|POST /ap/api/*`：agent-platform 兼容入口，网关转发前命中上游 `/api/*`
- `GET /ap/ws`：agent-platform WebSocket 入口，网关转发前改写为上游 `/ws`
...
- 当 `AP_BACKEND_MODE=container` 且上游容器未接入 `zenmind-network` 时，`/ap/api/*` 将在转发到上游 `/api/*` 时返回 `502`。
